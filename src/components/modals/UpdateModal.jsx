...
import {
    formatDateArrayToInputValue,
    formatMillisecondsTimestampToInputValue,
    formatUnixTimestampToInputValue
} from "../../utils/DataFormatter";
import {useState} from "react";
const UpdateModal = ({ onClose, onUpdate, record, loading, currentUser, coordinates, organizations, persons }) => {
    const { register, handleSubmit, setValue, formState: { errors } } = useForm({
        defaultValues: {
            ...record,
            creationDate: record.creationDate ? formatDateArrayToInputValue(record.creationDate) : '',
            startDate: record.startDate ? formatUnixTimestampToInputValue(record.startDate) : '',
            endDate: record.endDate ? formatDateArrayToInputValue(record.endDate) : '',
            person: {
                ...record.person,
                birthday: record.person?.birthday ? new Date(record.person.birthday).toISOString().split('T')[0] : ''
            }
        }
    });
    ...
    const [isOrgNull, setIsOrgNull] = useState(record.organization === null);
    ...
    const handleCoordinatesSelect = (coord) => {
        if (coord) {
            ...
            setValue('coordinates.x', record.coordinates.x);
            setValue('coordinates.y', record.coordinates.y);
        }
    };
    ...
    const handlePersonsSelect = (person) => {
        if (person) {
            ...
            setValue('person.eyeColor', record.person.eyeColor);
            setValue('person.hairColor', record.person.hairColor);
            setValue('person.location.x', record.person.location.x);
            setValue('person.location.y', record.person.location.y);
            setValue('person.location.z', record.person.location.z);
            setValue('person.location.name', record.person.location.name);
            setValue('person.birthday', formatMillisecondsTimestampToInputValue(record.person.birthday));
            setValue('person.height', record.person.height);
            setValue('person.passportID', record.person.passportID);
        }
    };
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white p-4 rounded shadow-lg w-[90%]">
                <h2 className="text-lg font-bold underline mb-2">Update Record</h2>
                                ...
                                onChange={(e) => {
                                    ...
                                    const coord = selectedId === "" ? null
                                        : currentUser.role === 'ADMIN' ?
                                            ...
                                            : coordinates.filter(c => c.author.id === currentUser.id).find(c => c.id === Number(selectedId));                                    handleCoordinatesSelect(coord);
